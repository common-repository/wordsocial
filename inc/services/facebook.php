...
class WSO_Facebook extends WSO_Service
{
	...
	private function internalCallApi($page, $argsOrType = array(), $argsIfType = array(), $postid = null)
	{
		...
		catch(FacebookApiException $e)
		{
			...
			wso_log_msg(sprintf(__('An error appeared with Facebook on this post id ($s) (Reason : %s)','wordsocial'), $postid, $e));
		}
		return 0;
	}
	...
	public function disconnect()
	{
		...
		update_option(WSO_OPTIONS, $wsoOpts);
        $urlPlugin = urlencode(str_replace('http://', '', get_bloginfo('wpurl')."/wp-admin/options-general.php?page=".WSO_PLUGIN_HOOK));
		$redir = "http://wso.li/iconnectFb.php?logout&r=". $urlPlugin;
		wp_redirect($redir);
	}
...
?>
<p><?php _e('Hello', 'wordsocial'); ?> <strong><?php echo $user['name']; ?></strong>. <a href="options-general.php?page=<?php echo WSO_PLUGIN_HOOK; ?>&disconnect=fb">[<?php _e('Disconnect', 'wordsocial'); ?>]</a></p>
