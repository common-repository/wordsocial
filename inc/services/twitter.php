...
class WSO_Twitter extends WSO_Service
{
	...
	public function connect()
	{
		...
		if(!isset($_GET['resp']))
		{
			$token = $this->m_OAuth->getRequestToken(get_bloginfo('wpurl')."/wp-admin/options-general.php?page=".WSO_PLUGIN_HOOK."&connect=tw&resp=1");
		...
		else
		{
			...
			session_unset();
			wp_redirect(get_bloginfo('wpurl')."/wp-admin/options-general.php?page=".WSO_PLUGIN_HOOK);
		}
	}
	...
	{	
		...
		{				
			...
			echo '<p><a href="options-general.php?page='.WSO_PLUGIN_HOOK.'&connect=tw"><img height="16" width="136" src="'. WSO_IMAGES_FOLDER .'signin-twitter.png" alt="signin twitter" /></a></p>';
		}
		else
		{
...
?>
<p><?php _e('Hello', 'wordsocial'); ?> <strong><?php echo $user->screen_name; ?></strong>. <a href="options-general.php?page=<?php echo WSO_PLUGIN_HOOK; ?>&disconnect=tw">[<?php _e('Disconnect', 'wordsocial'); ?>]</a></p>
