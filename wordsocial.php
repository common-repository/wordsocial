...
function wso_save_post($postid)
{
    ...
    // if revisions are enabled -> save before publish
    // if revisions are disabled -> publish before save
    // if revisions are disabled && scheduled -> save before publish
	$postParent = wp_is_post_revision($postid);
    
    //workaround
    $post = get_post($postid);
    if('future' == $post->post_status) $postParent = $postid;

	if($postParent)
	{		
		_wso_save_post($postParent);
	}
	else
	{
		//clear all postmeta
...
function _wso_save_post($postid)
{
	global $wso_services;
...
function wso_publish_post($postid)
{
    ...
    // if revisions are enabled -> save before publish
    // if revisions are disabled -> publish before save
    if(wp_is_post_revision($postid) === false && $_wso_opts === false)
    {
        _wso_save_post($postid);
    }
