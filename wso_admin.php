...
function wso_plugin_action_links( $links, $file ) {
	if ( $file == plugin_basename( dirname(__FILE__).'/wordsocial.php' ) ) {
		$links[] = '<a href="options-general.php?page='.WSO_PLUGIN_HOOK.'">'.__('Settings').'</a>';
	}
	return $links;
}
...
function wso_admin_menu()
{
	...
	$page = add_submenu_page('options-general.php', "WordSocial", "WordSocial", $allowedGroup, WSO_PLUGIN_HOOK, 'wso_admin_page');
...
function wso_admin_notices()
{
	...
	if(!empty($results)) :
		...
		$clear = __("<a href='options-general.php?page=".WSO_PLUGIN_HOOK."&wso_clear=1'>Clear all messages</a>", 'wordsocial');
...
function wso_inner_meta_boxes()
{
	...
	if(count($services) == 0)
	{
		echo "<p><a href='./options-general.php?page=".WSO_PLUGIN_HOOK."'>Please configure WordSocial</a>.</p>";
		return;
	}
...
<div class="wrap">
	...
	<form action="options-general.php?page=<?php echo WSO_PLUGIN_HOOK; ?>" method="post" name="form" id="wso_leftCont">
		<div id="wso-page">
			<ul>
...
<table class="form-table">
	<tbody>
		<tr>
			<td colspan="2">
                <?php _e("According to the setting (in Facebook/LinkedIn), it's possible to automatically publish an image with the post<br />WordSocial uses this priority to retrieve an image :", 'wordsocial'); ?>
                <ol>
                    <li><?php _e("Featured image (if your theme supports it)", 'wordsocial'); ?></li>
                    <li><?php _e("Image linked in the post (in the media gallery)", 'wordsocial'); ?></li>
                    <li><?php _e("Thumbnail from Youtube (if there is a youtube video)", 'wordsocial'); ?></li>
                    <li><?php _e("Image from the post (from a &lt;img&gt; tag)", 'wordsocial'); ?></li>
                </ol>
            </td>
		</tr>
		<tr>
